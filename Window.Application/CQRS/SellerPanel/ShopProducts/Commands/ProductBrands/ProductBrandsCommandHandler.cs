...
public record ProductBrandsCommandHandler : IRequestHandler<ProductBrandsCommand, bool>
{
    #region Ctor
    ...
    public ProductBrandsCommandHandler(IShopProductCommandRepository shopProductCommandRepository,
    ...
    public async Task<bool> Handle(ProductBrandsCommand request, CancellationToken cancellationToken)
    {
        #region Get Product By Id

        var product = await _shopProductQueryRepository.GetByIdAsync(cancellationToken , request.ProductId);
        if (product == null || product.SellerUserId != request.UserId) return false;
