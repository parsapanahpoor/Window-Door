...
namespace Window.Application.Services.Implementation
{
    public class PermissionService : IPermissionService
    {
        ...
        public WindowDbContext _context { get; set; }

        public PermissionService(WindowDbContext context , IUserService userServie )
        {
            _context = context;
        ...
        public async Task<GetUserRoles> GetUserRole(ulong userId)
        {
            ...
            if (userRoles.Any() && userRoles.Contains("SellerMaster")) return GetUserRoles.SellerMaster;
        ...
        public async Task<bool> IsUserSeller(ulong userId)
        {
            var result = await GetUserRole(userId);
            var user = await _userService.GetUserById(userId);
            ...
            if (result == GetUserRoles.Admin) return true;
            if (user.IsAdmin) return true;
            if (result == GetUserRoles.Seller) return true;
            if (result == GetUserRoles.SellerMaster) return true;

            return false;
        }

        public async Task<bool> IsUserMaster(ulong userId)
        {
            var result = await GetUserRole(userId);
            var user = await _userService.GetUserById(userId);
